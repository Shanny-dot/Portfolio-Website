...
export default function HeroSection() {
  const typewriterText = "Specializing in LLM evaluation, ETL pipeline assertions, hallucination testing, and automated guardrails. Now, what are we building?";
  ...
  return (
    <section id="hero" className="relative z-[1] h-screen flex flex-col justify-end pb-12 md:justify-center md:pb-0 px-5 sm:px-8 md:px-10 overflow-hidden">
      ...
      <div className="max-w-2xl relative z-10">
        ...
        >
          Hey there, I'm Shantanu Fadnavis,<br />
        ...
        >
          {whitePills.map((pill) => (
            <a
              key={pill.label}
              href={pill.href}
              className="inline-flex items-center justify-center bg-white text-black border border-black/10 rounded-full text-[13px] sm:text-[15px] px-4 sm:px-5 py-[0.3em] mx-[0.2em] mb-[0.4em] whitespace-nowrap hover:bg-emerald-400 hover:text-black transition-colors duration-200 cursor-pointer font-medium"
            >
              {pill.label}
            </a>
          ))}
          ...
          <button
            ...
            className="inline-flex items-center justify-center text-white bg-transparent border border-white/80 rounded-full text-[13px] sm:text-[15px] px-4 sm:px-5 py-[0.3em] mx-[0.2em] mb-[0.4em] whitespace-nowrap gap-2 sm:gap-3 hover:bg-white hover:text-black transition-colors duration-200 cursor-pointer"
          >
            <span>
              Reach me: <span className="underline underline-offset-1">{copied ? 'Copied to Clipboard!' : 'shantanufadnavis@protonmail.com'}</span>
            </span>
            <svg
